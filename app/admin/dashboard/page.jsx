...
export default function AdminDashboard() {
  ...
  return (
    <div className="bg-background text-on-surface font-body min-h-screen overflow-x-hidden select-none">
      ...
      {/* --- SideNavBar --- */}
      <aside
        className={`fixed top-0 left-0 h-full w-64 z-50 bg-[#131313] flex flex-col py-6 transition-transform duration-300 border-r border-white/5 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
      >
...
const SidebarLink = ({ icon, label, active, href }) => (
  <a
    className={`flex items-center space-x-3 rounded-[4px] px-4 py-3 mx-2 transition-all group ${active ? "text-white bg-[#C8102E]" : "text-[#e5e2e1] opacity-60 hover:bg-[#2a2a2a] hover:opacity-100"}`}
    href={href || "#"}
  >
